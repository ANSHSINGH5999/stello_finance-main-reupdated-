...
export class RiskEngine {
  ...
  private async runHealthCheck(): Promise<void> {
    ...
    const downValidators = validators.filter(
      (v: { uptime: number }) => v.uptime < config.protocol.validatorMinUptime
    );
    ...
    if (downValidators.length > validators.length * 0.3) {
      if (!this.emergencyMode) {
        ...
        await eventBus.publish(EventType.REBALANCE_REQUIRED, {
          reason: "emergency",
            validators: downValidators.map((v: { pubkey: string; allocatedStake: bigint }) => ({
            pubkey: v.pubkey,
  ...
  private async executeAutoReallocation(reason: string): Promise<void> {
    ...
    const unhealthy = validators.filter(
      (v: { uptime: number; allocatedStake: bigint }) => v.uptime < config.protocol.validatorMinUptime && v.allocatedStake > BigInt(0)
    );
    ...
    const stakeToRedistribute = unhealthy.reduce(
      (sum: bigint, v: { allocatedStake: bigint }) => sum + v.allocatedStake,
      BigInt(0)
    );
    ...
    const totalHealthyScore = healthy.reduce(
      (sum: number, v: { performanceScore: number }) => sum + v.performanceScore,
      0
    );
