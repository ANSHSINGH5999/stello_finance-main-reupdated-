...
export class MetricAggregator {
  ...
  async runUtilizationJob(windowStart: Date, windowEnd: Date): Promise<void> {
    ...
    const netBorrowDelta =
      BigInt(borrowSum._sum.amount ?? 0) -
      BigInt(repaySum._sum.amount ?? 0);

    if (netBorrowDelta !== 0n) {
      console.log(
        `[MetricAggregator] Utilization window net borrow delta: ${netBorrowDelta} stroops`
      );
    }

    const deposited = totalDeposited ?? 0n;
    const borrowed = totalBorrowed ?? 0n;
    const utilizationRate =
      deposited > 0n
        ? Number(borrowed) / Number(deposited)
        : 0;
    ...
    console.log(
      `[MetricAggregator] Utilization snapshot: deposited=${Number(deposited) / 1e7} ` +
        `borrowed=${Number(borrowed) / 1e7} rate=${(utilizationRate * 100).toFixed(2)}%`
    );
  }
  ...
  private async computeLendingRevenue(
    ...
    const liquidationBonus = liquidations.reduce((acc: bigint, ev: { collateralSeized: bigint; debtRepaid: bigint }) => {
  ...
  private async computeTreasuryRevenue(
    ...
    if (metricsBefore && metricsAfter) {
      const delta = BigInt(metricsAfter.protocolFees) - BigInt(metricsBefore.protocolFees);
