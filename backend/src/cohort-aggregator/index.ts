...
interface WalletDailyActivityRecord {
  wallet: string;
  stakedAmount: bigint;
...
export class CohortAggregator {
  ...
  async runBackfill(): Promise<void> {
    ...
    const existingSet = new Set(
      existingDateRows.map((r: { date: Date }) => dayFloor(r.date).toISOString())
    );
  ...
  async runWalletDailyActivityJob(date: Date): Promise<void> {
    ...
    const prevMap = new Map<string, WalletDailyActivityRecord>(
      prevActivity.map((r: WalletDailyActivityRecord) => [r.wallet, r])
    );
    ...
    const allWallets = new Set<string>([...prevMap.keys(), ...deltaMap.keys()]);
    const rows: Array<{
      wallet: string;
      date: Date;
      isStakeActive: boolean;
  ...
  async runCohortRetentionJob(targetDate: Date): Promise<void> {
    ...
    for (const cohortRow of cohortRows) {
      ...
      const walletIds = cohortWallets.map((w: { wallet: string }) => w.wallet);
    ...
    for (const cohortRow of cohortRows) {
      ...
      if (activePositions.length > 0) {
        ...
        const totalStaked = activePositions.reduce(
          (acc: number, p: { stakedAmount: bigint }) => acc + Number(p.stakedAmount),
          0
        );
        const totalBorrowed = activePositions.reduce(
          (acc: number, p: { borrowedAmount: bigint }) => acc + Number(p.borrowedAmount),
          0
        );
