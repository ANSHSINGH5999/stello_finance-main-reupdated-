...
async function getValidatorData(
  ...
  try {
    ...
    const avgScore =
      validators.reduce((sum: number, v: { performanceScore: number }) => sum + v.performanceScore, 0) /
      validators.length;
    const avgUptime =
      validators.reduce((sum: number, v: { uptime: number }) => sum + v.uptime, 0) / validators.length;
