...
export const statsRoutes: FastifyPluginAsync<{
  ...
  fastify.get<{ Querystring: { days?: string } }>("/chart-data", async (request) => {
    try {
      ...
      return {
        apyHistory: snapshots.map((s: { timestamp: Date; apy: number }) => ({ timestamp: s.timestamp.toISOString(), value: s.apy * 100 })),
        exchangeRateHistory: snapshots.map((s: { timestamp: Date; exchangeRate: number }) => ({ timestamp: s.timestamp.toISOString(), value: s.exchangeRate })),
        totalStakedHistory: snapshots.map((s: { timestamp: Date; totalStaked: bigint }) => ({ timestamp: s.timestamp.toISOString(), value: Number(s.totalStaked) / 1e7 })),
        tvlHistory: snapshots.map((s: { timestamp: Date; totalStaked: bigint }) => ({ timestamp: s.timestamp.toISOString(), value: (Number(s.totalStaked) / 1e7) * 0.12 })),
      };
    } catch {
