...
export const analyticsRoutes: FastifyPluginAsync<{ prisma: PrismaClient }> = async (
    ...
    async (request) => {
      try {
        ...
        return {
          ...
          to: to.toISOString(),
          data: snapshots.map((s: { timestamp: Date; totalStaked: bigint; exchangeRate: number }) => {
  ...
  fastify.get<{
    ...
    try {
      ...
      return {
        ...
        to: to.toISOString(),
        groupBy: "window",
        data: rows.map((r: { source: string; windowStart: Date; windowEnd: Date; amount: bigint }) => ({
          source: r.source,
          windowStart: r.windowStart.toISOString(),
          windowEnd: r.windowEnd.toISOString(),
          amountXlm: Number(r.amount) / 1e7,
  ...
  fastify.get<{
    ...
    try {
      ...
      const dateValues = cohortDates.map((c: { cohortDate: Date }) => c.cohortDate);
