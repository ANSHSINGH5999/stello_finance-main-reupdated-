...
export function registerErrorHandling(fastify: FastifyInstance): void {
  ...
  fastify.setErrorHandler((error, request, reply) => {
    ...
    if (isFastifyLikeError(error) && (error.validation || error.cause instanceof ZodError)) {
      const zodErr = error.cause instanceof ZodError ? error.cause : undefined;
      const response: ApiErrorResponse = {
        error: "Validation failed",
        code: "VALIDATION_ERROR",
        statusCode: 400,
    ...
    // Known error codes
    const mapped = isFastifyLikeError(error) ? ERROR_CODE_MAP[error.message] : undefined;
    if (mapped) {
      const response: ApiErrorResponse = {
        error: isFastifyLikeError(error) ? error.message : "Request failed",
        code: mapped.code,
    ...
    const statusCode = isFastifyLikeError(error) && error.statusCode && error.statusCode >= 400
      ? error.statusCode
      : 500;
