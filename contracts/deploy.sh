...
for wasm in "$TOKEN_WASM" "$STAKING_WASM" "$LENDING_WASM" "$LP_POOL_WASM" "$GOVERNANCE_WASM" "$TIMELOCK_WASM"; do
  if [ ! -f "$wasm" ]; then
...
# ---------------------------------------------------------------------------
# Step 6b: Deploy Timelock contract
# ---------------------------------------------------------------------------
echo "[6b] Deploying Timelock contract..."
...
# Timelock: 48 h delay (34560 ledgers at 5s/ledger), guardian = admin key initially
# GUARDIAN_PUB_KEY should be replaced with a multisig address before mainnet.
GUARDIAN_PUB_KEY="${GUARDIAN_PUBLIC_KEY:-$ADMIN_PUB_KEY}"
...
stellar contract invoke \
  ...
  --network "$NETWORK" \
  -- initialize \
  --admin "$GOVERNANCE_CONTRACT_ID" \
  --guardian "$GUARDIAN_PUB_KEY" \
  --delay_ledgers 34560

echo "  Timelock contract initialized (admin=governance, guardian=$GUARDIAN_PUB_KEY, delay=34560)"

# Wire governance → timelock
stellar contract invoke \
  --id "$GOVERNANCE_CONTRACT_ID" \
  --source "$ACCOUNT" \
  --network "$NETWORK" \
  -- set_timelock_contract \
  --admin "$ADMIN_PUB_KEY" \
  --timelock "$TIMELOCK_CONTRACT_ID"

echo "  Governance wired to timelock: $TIMELOCK_CONTRACT_ID"
...
# --- Governance Guardian ---
# Replace with a multisig address before mainnet deployment.
GUARDIAN_ADDRESS=$GUARDIAN_PUB_KEY
...
echo ""
echo "Governance / Timelock:"
echo "  Timelock delay: 34560 ledgers (~48h at 5s/ledger)"
echo "  Guardian: $GUARDIAN_PUB_KEY"
echo "  To override guardian: set GUARDIAN_PUBLIC_KEY env var before running deploy.sh"
echo ""
