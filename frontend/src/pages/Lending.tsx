    ...
    <div className="max-w-4xl mx-auto px-4 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Total Collateral', value: formatXLM(stats.totalCollateral) + ' XLM' },
          { label: 'Collateral Assets', value: String(stats.collateralAssets.length) },
      ...
      <div className="grid md:grid-cols-2 gap-6">
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              ...
              <div className="pt-2 border-t border-white/5 space-y-2">
                <p className="text-xs text-gray-400">Collateral Assets</p>
                  ...
                  position.collateralAssets.map((asset) => (
                    <div key={asset.assetId} className="flex justify-between text-sm">
                      ...
                      <span className="text-white">
                        {formatXLM(asset.balance)} ({formatXLM(asset.collateralValueXlm)} XLM)
                      </span>
                    </div>
                  ))
                ) : (
                  <p className="text-sm text-gray-500">No collateral deposited</p>
                )}
              </div>
            </div>
          )}
        ...
        <div className="glass rounded-2xl p-6 space-y-4">
            ...
            <div>
              ...
              >
                {availableAssets.map((asset) => (
                  <option key={asset.assetId} value={asset.assetId} className="bg-slate-900">
                    {asset.symbol} | CF {(asset.collateralFactorBps / 100).toFixed(0)}% | Price {asset.price.toFixed(4)} XLM
                  </option>
                ))}
              </select>
            </div>
          ...
          {(activeTab === 'borrow' || activeTab === 'withdraw' || activeTab === 'repay') && position.collateralAssets.length === 0 && (
            <div className="rounded-lg p-3 text-xs" style={{ background: 'rgba(245,207,0,0.06)', border: '1px solid rgba(245,207,0,0.2)' }}>
              ...
              <p className="text-gray-400">You have no collateral deposited. Switch to the <strong className="text-white">Deposit</strong> tab, choose an enabled asset, then come back to borrow.</p>
            </div>
          )}
          ...
          {activeTab === 'liquidate' ? (
            ...
            <div>
              <label className="text-xs text-gray-400 mb-1 block">
                {activeTab === 'deposit' || activeTab === 'withdraw'
                  ? `${availableAssets.find((asset) => asset.assetId === selectedAssetId)?.symbol ?? 'Collateral'} Amount`
                  : 'XLM Amount'}
              </label>
          ...
          {isConnected ? (
            <button
              ...
              disabled={
                ...
                (['borrow', 'withdraw', 'repay'].includes(activeTab) && position.collateralAssets.length === 0)
              }
      ...
      <div className="glass rounded-2xl p-6 space-y-4">
        ...
        <div className="space-y-3 text-sm text-gray-400">
          <p>1. Deposit one of the enabled collateral assets into the lending contract.</p>
          <p>2. Each asset has its own collateral factor, which determines how much you can borrow against it.</p>
